...
ad_data$analysis_data %>% select(ID_merged,Date_timeline,TRAILS,CUBE,CLOCKCON)
...
```{r}
src_table
`MOCA_15Feb2024 1.08.40 PM` %>% filter(RID == 8)
```
...
ad_data = ad_merge(biocard_path,dict_src = src_table,timeline_file = "BIOCARD_CognitiveData_2021.11.02")
...
### found BUG the date type input is a vector, which cause the multiple comparison of the if statement.
```
...
ad_data$analysis_data %>% select(ID_merged,Date_timeline,DIAGNOSIS,DXNODEP)
